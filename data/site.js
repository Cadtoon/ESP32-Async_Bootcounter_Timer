...
the value in the grey area is updated before sending it the the ESP */
	function updateAlarmonoff(val) {
   	...
   	function updateTimer1switch(val) {
          document.getElementById('Timer1switch').value=val; 
        }
   	function updateTimer2switch(val) {
          document.getElementById('Timer2switch').value=val; 
        }
...
If you sk me what each line of code is doing? Ya, I don't have the answer. */
	...
	function getESPDate() {
   	...
   	function getESPBOOTS() {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            document.getElementById("ESPBOOTS").innerHTML = this.responseText;}
          };  
        xhttp.open("GET", "/ESPBOOTS", true);
        xhttp.send(); 
      }
   	function getESPBOOTDATE() {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            document.getElementById("ESPBOOTDATE").innerHTML = this.responseText;}
          };  
        xhttp.open("GET", "/ESPBOOTDATE", true);
        xhttp.send(); 
      }
