/****************************************************************************************************************************
  ESP32-Async_Bootcounter_Timer.ino (For ESP32 boards)
  ...
  The sketch and files included in this project contain features:
      ...
      * Email notification on Alarm setting
      * Two timers where one timer turns GPIO 02 LED on and off
  ...
  Instructions:
    ...
    Additional information provided on webpages. 
...
// needed to save variables to eeprom
#include <Preferences.h> // https://github.com/espressif/arduino-esp32
...
SMTPData smtpData; // required to send emails
Preferences preferences; // required to save data to eeprom
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
void BootCounter(){ // This function is called from the setup();
  preferences.begin("my-app", false); //open or create a namespace called my-app. Can be any name less then 15 characters.
  // You can only access eeprom after a preferences.begin starts the function.
  unsigned int counter = preferences.getUInt("counter", 0); //get or create key called counter. If created the default value is set to 0
  String thisdate = preferences.getString("bootdate",""); //get or create key called bootdate. If created the default value is set to "", a blank value
  counter++; //increment counter
  bootdate = thisdate; //set global variable
  bootcounter = counter; //set global variable
  preferences.putUInt("counter", counter); //store new counter value
  Serial.print("Rebooted ");
  Serial.print(counter);
  Serial.print(" times since ");
  Serial.println(bootdate);
  preferences.end(); // Close the preferences. you must close the preferences function when done
}

//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
void ResetBootCounter(){ // This function is activated from a webpage button "Reset Counter"
  preferences.begin("my-app", false); //Open the namespace my-app
  preferences.clear(); //clear all the content
  String thisdate = getESPtimedate("counterdate"); // get a new reset date
  int counter =0; //reset counter to 0
  preferences.putUInt("counter", counter); //store new counter value
  preferences.putString("bootdate",thisdate); //store new counter date
  bootdate = thisdate; //reset global variable used to display on webpages
  bootcounter = counter; //reset global variable used to display on webpages
...
// new files for saving timer settings
  timer1_on = readFile(SPIFFS, "/Timer1on.txt");
  Serial.print("*** timer1_on: "); Serial.println(timer1_on);
  timer1_off = readFile(SPIFFS, "/Timer1off.txt");
  Serial.print("*** timer1_off: "); Serial.println(timer1_off);
  timer1_switch  = readFile(SPIFFS, "/Timer1switch.txt").toInt();
  Serial.print("*** timer1_switch: "); Serial.println(timer1_switch);
  timer2_on = readFile(SPIFFS, "/Timer2on.txt");
  Serial.print("*** timer2_on: "); Serial.println(timer2_on);
  timer2_off = readFile(SPIFFS, "/Timer2off.txt");
  Serial.print("*** timer2_off: "); Serial.println(timer2_off);
  timer2_switch  = readFile(SPIFFS, "/Timer2switch.txt").toInt();
  Serial.print("*** timer2_switch: "); Serial.println(timer2_switch);
...
// include the new timers
  else if(var == "Timer1on") return readFile(SPIFFS, "/Timer1on.txt");
  else if(var == "Timer1off") return readFile(SPIFFS, "/Timer1off.txt");
  else if(var == "Timer1switch") return readFile(SPIFFS, "/Timer1switch.txt");
  else if(var == "Timer2on") return readFile(SPIFFS, "/Timer2on.txt");
  else if(var == "Timer2off") return readFile(SPIFFS, "/Timer2off.txt");
  else if(var == "Timer2switch") return readFile(SPIFFS, "/Timer2switch.txt");
  return String();
}
...
// new to include reboot data
  webserver.on("/ESPBOOTS", HTTP_GET, [](AsyncWebServerRequest *request){
    String countAsString = String(bootcounter); //convert to a string.
    request->send(200, "text/plain", countAsString); //send back converted string
  });
  webserver.on("/ESPBOOTDATE", HTTP_GET, [](AsyncWebServerRequest *request){
    String bootdateAsString = String(bootdate); //convert to a string.
    request->send(200, "text/plain", bootdateAsString); //send back converted string
  });
// handle webpage requests, this handles the use of multiple webpages
  ...
  webserver.on("/timers.html", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/timers.html", String(), false, processor);  //get the requested file from SPIFFS storage and use the processor function to fill in the place holders
  });
...
// handle changes made to the new timers
      else if (request->hasParam(PARAM_TIMER1ON)) {
      inputMessage = request->getParam(PARAM_TIMER1ON)->value();
      writeFile(SPIFFS, "/Timer1on.txt", inputMessage.c_str());
      timer1_on = inputMessage;
    }
      else if (request->hasParam(PARAM_TIMER1OFF)) {
      inputMessage = request->getParam(PARAM_TIMER1OFF)->value();
      writeFile(SPIFFS, "/Timer1off.txt", inputMessage.c_str());
      timer1_off = inputMessage;
    }
      else if (request->hasParam(PARAM_TIME1SWITCH)) {
      inputMessage = request->getParam(PARAM_TIME1SWITCH)->value();
      writeFile(SPIFFS, "/Timer1switch.txt", inputMessage.c_str());
      timer1_switch = inputMessage.toInt();
    }
      else if (request->hasParam(PARAM_TIMER2ON)) {
      inputMessage = request->getParam(PARAM_TIMER2ON)->value();
      writeFile(SPIFFS, "/Timer2on.txt", inputMessage.c_str());
      timer2_on = inputMessage;
    }
      else if (request->hasParam(PARAM_TIMER2OFF)) {
      inputMessage = request->getParam(PARAM_TIMER2OFF)->value();
      writeFile(SPIFFS, "/Timer2off.txt", inputMessage.c_str());
      timer2_off = inputMessage;
    }
      else if (request->hasParam(PARAM_TIME2SWITCH)) {
      inputMessage = request->getParam(PARAM_TIME2SWITCH)->value();
      writeFile(SPIFFS, "/Timer2switch.txt", inputMessage.c_str());
      timer2_switch = inputMessage.toInt();
    }
// new to include when the reset counter button is pressed
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// a new function to check the timers and alarm. If there is a match a trigger for each time is set
void  checkTIMERS(){
  struct tm timeinfo;
  if(WiFi.status() == WL_CONNECTED){//if the Wi-Fi has a connection than we should have NTP time
    if(!getLocalTime(&timeinfo)){ //get the time on the ESP
      Serial.println("Failed Time Request");
      return; //just in case the NTP failed
    }
    uint8_t seconds = timeinfo.tm_sec;  // extract seconds
    if(seconds == 0){
      uint8_t hours = timeinfo.tm_hour; // extract hours
      uint8_t minutes = timeinfo.tm_min; // extract minutes
      uint8_t moday = timeinfo.tm_mday;  // extract day of the month 
      if(moday == 1 or moday == 15 && hours == 0 && minutes == 0){ //update clock time twice a month
        GetNTPtime();
      }         
      char timeStringBuff[50];
      strftime(timeStringBuff, sizeof(timeStringBuff), "%H:%M", &timeinfo);
...
//      delay(1010);
      if(timer1_on != timer1_off){ //if these are equal timer1 is disabled
        if(timer1_switch){
          if(currenttime == timer1_on){ 
            timer1on_trigger = 1; //turns on trigger
          }
          if(currenttime == timer1_off){ 
            timer1off_trigger = 1; //turns on trigger
          }
        }
      }
      if(timer2_on != timer2_off){ //if these are equal timer2 is disabled
        if(timer2_switch){
          if(currenttime == timer2_on){ 
            timer2on_trigger = 1; //turns on trigger
          }
          if(currenttime == timer2_off){ 
            timer2off_trigger = 1; //turns on trigger
          }
        }
      }
      if(alarmonoff){ 
        if(currenttime == alarmtime){ 
          alarm_trigger = 1; //turns on trigger
        }
      }
    }
  }
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
// a new function to check the triggers, if found perform an action
void  checkTRIGGERS(){
  if(timer1on_trigger == 1){
    Serial.println("timer1_on triggered"); // turns on the LED
    digitalWrite(LED,HIGH);
    timer1on_trigger = 0; //turns off trigger
  }
  if(timer1off_trigger == 1){
    Serial.println("timer1_off triggered"); // turns off the LED
    digitalWrite(LED,LOW);
    timer1off_trigger = 0; //turns off trigger
  }
  if(timer2on_trigger == 1){
    Serial.println("timer2_on triggered"); // does nothing
    timer2on_trigger = 0; //turns off trigger
  }
  if(timer2off_trigger == 1){
    Serial.println("timer2_off triggered"); // does nothing
    timer2off_trigger = 0; //turns off trigger
  }
  if(alarm_trigger == 1){
    Serial.println("alarm triggered"); // sends an email
    smtpData.setMessage("ESP Alarm Triggered", false);
    if(!MailClient.sendMail(smtpData)){
      Serial.println("Error sending Email, " + MailClient.smtpErrorReason());
    }
    alarm_trigger = 0; //turns off trigger
  }
}
...
// call the new boot counter function
  BootCounter();
